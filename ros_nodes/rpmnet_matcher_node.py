    ...
    def _run_rpmnet(self, src_scan: PreparedScan, ref_scan: PreparedScan):
        ...
        if not self.args.pose_only:
            ...
            if self.args.hungarian_matches:
                ...
                threshold = perm_np[row_ind, col_ind].max() / 1.3
                keep_mask = perm_np[row_ind, col_ind] > threshold
                ...
                weights[row_ind] = perm_np[row_ind, col_ind]
...
def run_live(args, matcher: RpmnetMatcher):
...
if __name__ == "__main__":
    cli_args = parse_args()
    matcher = RpmnetMatcher(cli_args)
    run_live(cli_args, matcher)
